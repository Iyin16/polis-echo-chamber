...
export function MemoryTimeline() {
  ...
  const [memoryArchiveState, setMemoryArchiveState] = useState<
    Record<string, { archived: boolean; rootHash?: string; loading?: boolean }>
  >(() =>
    Object.fromEntries(
      memories.map((memory) => [
        memory.slug,
        { archived: memory.archivedOn0g ?? false, rootHash: undefined, loading: false },
      ]),
    ),
  );
    ...
    <>
      <section className="px-4 md:px-6 py-10">
        <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
          <div>
            ...
            <h2 className="font-serif text-xl md:text-2xl tracking-tight mt-1">
              Governance Eras of Polis
            </h2>
            <p className="text-[12.5px] text-muted-foreground mt-1 max-w-xl">
              Six discrete political epochs the chamber has lived through. Each era's doctrine still
              shapes contemporary deliberation.
            </p>
          </div>
          <span className="font-mono text-[10px] text-muted-foreground hidden md:inline">
            Cycle 1 → Cycle 31 · 31 cycles indexed
          </span>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            {eras.map((e, i) => (
              <article key={e.id} className="relative">
                ...
                <span
                  className={`absolute -top-[3px] left-0 h-1.5 w-1.5 rounded-full ${i === eras.length - 1 ? "bg-amber glow-amber" : i % 2 ? "bg-cyan" : "bg-silver"}`}
                />
                <div className="panel rounded-md p-3 mt-1">
                  ...
                  <p className="mt-2 text-[10.5px] uppercase tracking-[0.14em] text-cyan/80 font-mono">
                    {e.doctrine}
                  </p>
                  <p className="text-[11.5px] text-foreground/75 mt-2 leading-relaxed line-clamp-4">
                    <EntityText>{e.summary}</EntityText>
                  </p>
                  <ul className="mt-2 space-y-0.5">
                    {e.defining.slice(0, 2).map((d, j) => (
                      <li key={j} className="flex gap-1.5 text-[10.5px] text-muted-foreground">
      ...
      <section className="px-4 md:px-6 py-8 border-t hairline">
        <div className="mb-4">
          ...
          <h2 className="font-serif text-xl md:text-2xl tracking-tight mt-1">
            Major Governance Replays
          </h2>
          <p className="text-[12.5px] text-muted-foreground mt-1 max-w-xl">
            Chronological moments from chamber memory, replayed as compact event fragments with
            agent reactions and infrastructure proof status.
          </p>
        </div>
      ...
      <section className="px-4 md:px-6 py-8 border-t hairline">
        <div className="mb-4">
          ...
          <h2 className="font-serif text-xl md:text-2xl tracking-tight mt-1">
            Treaties &amp; Constitutional Instruments
          </h2>
        </div>
          ...
          {treaties.map((t) => (
            <div key={t.id} className="grid grid-cols-12 gap-3 px-4 py-3 items-start">
              ...
              <div className="col-span-3 md:col-span-2 flex md:justify-end">
                <span
                  className={`rounded-sm border px-1.5 py-0.5 text-[9.5px] uppercase tracking-[0.14em] ${treatyColor[t.status]}`}
                >
                  {t.status}
                </span>
              </div>
      ...
      <section className="px-4 md:px-6 py-10 border-t hairline">
        <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-[12.5px] text-muted-foreground mt-1 max-w-xl">
              Events the chamber refuses to forget. Every agent references this archive when forming
              positions.
            </p>
          </div>
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            {memories.map((m, i) => {
              const state = memoryArchiveState[m.slug] ?? {
                archived: m.archivedOn0g ?? false,
                rootHash: undefined,
                loading: false,
              };
              return (
                <article key={m.id} className="relative">
                  ...
                  >
                    <div className="flex items-center gap-2 h-[42px]">
                      ...
                      <span
                        className={`ml-auto rounded-sm border px-1.5 py-0.5 text-[9.5px] uppercase tracking-[0.14em] ${catColor[m.category]}`}
                      >
                        {m.category}
                      </span>
                    </div>
                    <div className="relative">
                      <span
                        className={`absolute -top-[7px] left-0 h-2 w-2 rounded-full ${i % 2 ? "bg-cyan" : "bg-amber"}`}
                      />
                      <div className="panel rounded-md p-3.5 mt-1 group-hover:border-[color-mix(in_oklab,var(--silver)_22%,transparent)] transition-colors">
                        ...
                        <p className="text-[12px] text-foreground/75 mt-2 leading-relaxed line-clamp-4">
                          {m.summary}
                        </p>
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2 text-[11px]">
                    <span className="text-muted-foreground">
                      {state.rootHash
                        ? `0G root: ${state.rootHash}`
                        : "Preserve this event in permanent memory."}
                    </span>
                    <button
                      type="button"
